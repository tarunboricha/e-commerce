...
export class ProductSerService {
  ...
  AddProductservice(data: product) {
    return this.http.post('http://localhost:5000/products', data, { headers: this.headers, observe: 'response' });
  }

  productListservice() {
    return this.http.get<product[]>('http://localhost:5000/products', { headers: this.headers });
  }

  deleteProductservice(data: number) {
    return this.http.delete(`http://localhost:5000/products/${data}`, { headers: this.headers });
  }

  getProductservice(data: string) {
    return this.http.get<product[]>(`http://localhost:5000/products/${data}`, { headers: this.headers });
  }

  updateProductservice(data: product) {
    return this.http.put(`http://localhost:5000/products/${data.id}`, data, { headers: this.headers });
  }

  popularProductservice() {
    return this.http.get<product[]>('http://localhost:5000/popular_products', { headers: this.headers });
  }

  trendingProductservice() {
    return this.http.get<product[]>('http://localhost:5000/trending_products', { headers: this.headers });
  }

  searchSuggestionservice(data: string) {
    return this.http.get<product[]>(`http://localhost:5000/products?q=${data}`, { headers: this.headers });
  }

  searchProductService(data: string) {
    return this.http.get<product[]>(`http://localhost:5000/products/productType/${data}`, { headers: this.headers });
  }
  ...
  UseraddTocart(data: addToCart) {
    return this.http.post('http://localhost:5000/Cart', data, { headers: this.headers });
  }

  UseraddTocarts(data: addToCart[]) {
    return this.http.post('http://localhost:5000/Carts', data, { headers: this.headers });
  }

  userremoveTocart(data: number, userID: number) {
    return this.http.delete('http://localhost:5000/Cart/' + data + '/' + userID, { headers: this.headers });
  }
  ...
  getCartlist(data: number, fun:string) {
    ...
    this.isLoader = true;
    return this.http.get<product[]>(`http://localhost:5000/Cart/${data}`,
